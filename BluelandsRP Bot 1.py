...
async def on_ready():
    ...
    await client.change_presence(game=discord.Game(name='!website - !info - !news'))
...
@client.command(pass_context=True)
async def news(ctx):
    await client.say("""**BluelandsRP News**
Our website is getting updates!
The BluelandsRP Bot is getting updates!
New page on the website! 'News'
https://bluelandsrp.wixsite.com/official/news
BluelandsRP Is looking for Discord Moderators! PM @Founder or @Co-Founder for more Info!
https://bluelandsrp.wixsite.com/official
**-BluelandsRP News**""")
...
!news - Gives you the latest BluelandsRP News!
...
async def warn(ctx, *, member : discord.Member = None):
    ...
    if not member:
        return await client.say(ctx.message.author.mention + "Specify a user to warn!")
    try:
        await client.warn(member)
    except Exception as e:
        if 'Privilege is too low' in str(e):
            return await client.say(":x: Privilege too low!")
 
    embed = discord.Embed(description = "**%s** has been warned!"%member.name, color = 0xFF0000)
    return await client.say(embed = embed)
